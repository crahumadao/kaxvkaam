...
def rulpawe(txt):			#UNI	AZÜ
	...
	txt=re.sub('(?<![Nn])g','ng',txt)	#Ng,g	G,g	
	txt=re.sub('(?<![Nn])G','Ng',txt)	#Ng,g	G,g	
	...
	#txt=re.sub()			#K,k	K,k
	#txt=re.sub()			#L,l	L,l
	txt=re.sub('lh','l_',txt)	#L_,l_	Lh,lh
	txt=re.sub('Lh','L_',txt)	#L_,l_	Lh,lh
	...
	#txt=re.sub()			#N,n	N,n
	#txt=re.sub()			#Ñ,ñ	Ñ,ñ
	txt=re.sub('nh','n_',txt)	#N_,n_	Nh,nh
	txt=re.sub('Nh','N_',txt)	#N_,n_	Nh,nh
	#txt=re.sub()			#O,o	O,o
	#txt=re.sub()			#P,p	P,p
	#txt=re.sub()			#R,r	R,r
	...
	#txt=re.sub()			#---	Sh,sh
	#txt=re.sub()			#T,t	T,t
	txt=re.sub('tx','tr',txt)	#Tr,tr	Tx,tx
	txt=re.sub('Tx','Tr',txt)	#Tr,tr	Tx,tx
	txt=re.sub('t\'','t_',txt)	#T_,t_	T',t'
	txt=re.sub('T\'','T_',txt)	#T_,t_	T',t'
	#txt=re.sub()			#U,u	U,u
	#txt=re.sub()			#Ü,ü	Ü,ü
	#txt=re.sub()			#W,w	W,w
